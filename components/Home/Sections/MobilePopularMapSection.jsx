...
export default function MobilePopularMapSection({ index = 2 }) {
    const section = {
        title: 'Beloved Lands,',
        subtitle: 'Beloved Legends',
    ...
    return (
        <div className="relative capitalize">
            ...
            <div className={'relative h-screen flex flex-col justify-center items-center text-white text-center'}>
                <div className="absolute inset-0 bg-[url('/images/home/xl/mobile/mobile-pokemon-popular-xl.png')] bg-repeat-round bg-contain z-0"></div>

                <div className="flex flex-col gap-20 z-10 justify-center h-full items-center py-[25%] w-full">
                    <div className="flex flex-col gap-4 items-center justify-center w-full">
                        <div className="flex flex-col">
                            <h1 className="text-xl">{section.title}</h1>
                            <h1 className="text-2xl">{section.subtitle}</h1>
                        </div>
                        <p className="text-3xs max-w-2xl px-[15%]">{section.desc}</p>
                    </div>

                    <div className="w-full flex gap-8 h-fit overflow-x-auto pl-[10%] scrollbar-hide">
                        {mapImage.map((map) => (
                            <div
                                key={map.name}
                                className="flex flex-col items-center justify-center border-white rounded-2xl border-2 relative "
                            >
                                <div className="absolute bottom-0 left-0 w-full bg-black/50 !rounded-tl-sm !rounded-tr-sm border-t-1 border-white flex items-center justify-center h-fit flex-col gap-2 px-2 z-10 rounded-2xl py-2">
                                    <p className="text-white uppercase text-center text-3xs">{map.name}</p>
                    ...
                    <p className="text-3xs px-[15%]">{section.secondary_desc}</p>
                </div>
            </div>
        </div>
    );
